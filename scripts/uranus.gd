...
@export var orbit_radius: float = 2200.0
@export var orbit_period: float = 364.0
@export var start_angle: float = 1.5

const G: float = 1.0

var sun_mass: float = 1.0
var mass: float = 14.5
var collision_radius: float = 56.0
var _pos: Vector2
var _vel: Vector2
	...
	for x in range(size):
		for y in range(size):
			...
			if dist <= max_r:
				...
				var r := 0.5 * brightness
				var g := 0.8 * brightness
