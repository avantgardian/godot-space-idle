...
const PAL_T: GDScript = preload("res://scripts/util/tron_palette.gd")
const PAL_P: GDScript = preload("res://scripts/util/planet_palette.gd")
const DU: GDScript = preload("res://scripts/util/draw_utils.gd")
const ASTEROID_SHADER: Shader = preload("res://shaders/bodies/asteroid_surface.gdshader")
...
const TEXTURE_SIZE: int = 24

const ARCHETYPE_WEIGHTS: Array[float] = [0.55, 0.25, 0.10, 0.10]
...
func _generate_texture() -> void:
	...
	for i: int in range(ARCHETYPE_WEIGHTS.size()):
...
func spawn() -> void:
	...
	var m_norm: float = clampf((mass - 1.5e-8) / (6e-8 - 1.5e-8), 0.0, 1.0)
	_visual_radius_px = lerpf(2.0, 10.0, pow(m_norm, 1.0 / 3.0))

	collision_radius = _visual_radius_px * 0.7

	var mass_norm_mid: float = mass / 6e-8
	_spin_rate = 0.5 / clamp(mass_norm_mid, 0.25, 1.0)
	if randf() < 0.5:
		_spin_rate = -_spin_rate

	var scl: float = _visual_radius_px / (TEXTURE_SIZE * 0.5)
	_sprite.scale = Vector2(scl, scl)

	_shader_mat.set_shader_parameter("u_spin_rate", _spin_rate)

	var spawn_r: float = randf_range(2400.0, 3200.0)
	var entry_angle: float = randf_range(0.0, TAU)
	_pos = Vector2(cos(entry_angle), sin(entry_angle)) * spawn_r

	var v_inf: float = randf_range(5.0, 120.0)
	var radial_frac: float = randf_range(0.5, 0.95)
	var radial: float = -v_inf * radial_frac
	var tangential: float = v_inf * sqrt(1.0 - radial_frac * radial_frac)
	var dir: Vector2 = Vector2(cos(entry_angle), sin(entry_angle))
	var tangent: Vector2 = Vector2(-dir.y, dir.x) * (1.0 if randf() < 0.5 else -1.0)
