...
@export var orbit_radius: float = 1400.0
@export var orbit_period: float = 221.0
@export var start_angle: float = 0.5

const G: float = 1.0

var sun_mass: float = 1.0
var mass: float = 317.8
var collision_radius: float = 75.0
var _pos: Vector2
var _vel: Vector2
...
func _generate_texture():
	...
	for x in range(size):
		for y in range(size):
			...
			if dist <= max_r:
				var t := dist / max_r
				var band := sin(float(y) * 0.7) * 0.15
				var brightness := 0.65 + 0.35 * (1.0 - t) + band
				var r := (0.85 + band * 0.3) * brightness
				var g := (0.65 + band * 0.2) * brightness
				var b := (0.3 - band * 0.1) * brightness
				var alpha := 1.0
				if t > 0.85:
