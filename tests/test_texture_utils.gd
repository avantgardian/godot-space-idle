...
func test_make_noisy_blob_returns_image_texture():
	var tex: ImageTexture = TEX.make_noisy_blob(16, 42, func(_t: float, _x: int, _y: int) -> Color: return Color.WHITE)
	assert_true(tex is ImageTexture, "Returns ImageTexture")

func test_make_noisy_blob_corners_transparent():
	var tex: ImageTexture = TEX.make_noisy_blob(16, 99, func(_t: float, _x: int, _y: int) -> Color: return Color.RED)
	var image: Image = tex.get_image()
	assert_eq(image.get_pixel(0, 0), Color.TRANSPARENT, "Top-left corner is transparent")
...
func test_make_noisy_blob_center_opaque():
	var seed_val := 42
	var tex: ImageTexture = TEX.make_noisy_blob(32, seed_val, func(_t: float, _x: int, _y: int) -> Color: return Color(0.8, 0.6, 0.4, 1.0))
	var image: Image = tex.get_image()
...
func test_make_noisy_blob_uses_color_fn():
	...
	var tex: ImageTexture = TEX.make_noisy_blob(32, 12345, func(_t: float, _x: int, _y: int) -> Color: return test_color)
	var image: Image = tex.get_image()
	var found := false
	for x in range(32):
		for y in range(32):
...
func test_make_noisy_blob_no_inline_color_literals():
	var seed_val := 77
	var tex: ImageTexture = TEX.make_noisy_blob(32, seed_val, func(_t: float, _x: int, _y: int) -> Color:
...
func test_make_noisy_blob_idempotent():
	...
	for x in range(16):
		for y in range(16):
			assert_eq(img1.get_pixel(x, y), img2.get_pixel(x, y), "Same seed produces same texture at (%d,%d)" % [x, y])
